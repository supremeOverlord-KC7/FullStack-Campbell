...
def get_image():
    ...
    for child in jsondata['data']['children']:
        image_l.append(child['data']['preview']['images'][0]['source']['url'])
    
    random.shuffle(image_l)


    return jsonify({"success":True,"url":image_l[0]})
...
def get_quote():
    ...
    for child in jsondata['data']['children']:
        quote_l.append(child['data']['title'])
    
    random.shuffle(quote_l)


    return jsonify({"success":True,"quote":quote_l[0]})
